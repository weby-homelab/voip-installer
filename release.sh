...
sed -i "s/Version:** \`v${CURRENT_VER}\`/Version:** \`v${NEW_VER}\`/" README.md
sed -i "s/Версия:** \`v${CURRENT_VER}\`/Версия:** \`v${NEW_VER}\`/" README_RUS.md
sed -i "s/Версія:** \`v${CURRENT_VER}\`/Версія:** \`v${NEW_VER}\`/" README_UKR.md
...
echo "git add . && git commit -m \"feat: release v${NEW_VER}\" && git tag v${NEW_VER} && git push && git push --tags"
